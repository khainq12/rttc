...
certificates (`certificate_expired_rejected_claim`,
`certificate_not_yet_valid_rejected_claim`). Checked first what was
already covered rather than assuming a gap: CA rollover was already
proven both directions (`docker_ngtcp2_public_online_client_ca_rotation_5run_probe`
rotates CA-A to CA-B, confirms both accept/reject flip correctly, *then
restores CA-A and confirms it works again* -- a genuine rollback, not
just an addition), CRL revocation/refresh already fails closed on
malformed replacement, and server/client-CA rotation were already done.
What had never been generated by any existing probe was a certificate
whose CA, subject, and required URI SAN are all otherwise exactly like
the passing "valid" certificate, differing only in its temporal validity
window -- `docker_ngtcp2_public_certificate_temporal_validity_probe.py`
closes that specifically, using `cryptography`'s `CertificateBuilder`
(the same library this suite's CRL generation already relies on) to set
an explicit `not_valid_before`/`not_valid_after` window: one certificate
expired a day ago, one not valid until tomorrow. Since the temporal check
inside `gnutls_certificate_verify_peers3` is `now >= notBefore && now <=
notAfter` regardless of which side of that inequality moved, this is the
same verification path any real client/server clock-skew scenario would
exercise -- proven this way specifically to avoid touching the shared
machine's actual wall clock (containers don't have an isolated
`CLOCK_REALTIME`, so `date -s` inside one would skew the host too). Both
certificates are rejected, isolating the rejection to the temporal check
alone since every other property matches the passing case. 5/5 runs.
Crash-consistency under a hard kill is not a new gap either: it's already
what the existing Raft and etcd/PostgreSQL multi-host fencing probes
above prove directly (`docker kill`/QEMU-process-kill mid-operation, then
independently verifying no double-applied writes and no divergent
