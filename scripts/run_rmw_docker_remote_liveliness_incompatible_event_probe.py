"""Build and run the FleetRMW remote liveliness-incompatible-QoS event probe
in separate Docker containers, over a real UDP wire with netem impairment.

This closes the one concrete, bounded gap identified for the liveliness/
remote-graph-event claims: the incompatible-QoS-event code path
(incompatible_qos_policy_kind() in rmw_pubsub.cpp) is shared verbatim between
local match-checking and remote-endpoint discovery, but had no dedicated
test exercising it for LIVELINESS specifically across a real two-process
wire boundary (see qos_liveliness_incompatible_event_probe.cpp for the
same-process case, and remote_event_probe.cpp for remote coverage of
reliability/durability/deadline, but never liveliness).
"""
...
SCHEMA_VERSION = "fleetrmw.docker_remote_liveliness_incompatible_event_probe.v1"
...
def run_case(
    *,
    ...
    observer = f"fleetrmw-remote-liveliness-observer-{os.getpid()}-{index}"
    advertiser = f"fleetrmw-remote-liveliness-advertiser-{os.getpid()}-{index}"
    binary = (
        f"{install}/rmw_fleetqox_cpp/lib/rmw_fleetqox_cpp/"
        "fleetrmw_remote_liveliness_incompatible_event_probe"
    )
    observer_command = (
        f"source /opt/ros/jazzy/setup.bash && source {install}/setup.bash && "
        "tc qdisc replace dev eth0 root netem delay 5ms 1ms && "
        f"FLEETQOX_RMW_BIND=0.0.0.0:48420 {binary} --mode observer --timeout-ms 9500"
    )
    start = run_command(
        [
            "docker",
    ...
    try:
        if ready:
            advertiser_command = (
                f"source /opt/ros/jazzy/setup.bash && source {install}/setup.bash && "
                "tc qdisc replace dev eth0 root netem delay 5ms 1ms && "
                "FLEETQOX_RMW_BIND=0.0.0.0:48421 "
                f"FLEETQOX_RMW_PEERS={observer}:48420 "
                f"{binary} --mode advertiser --hold-ms 2200"
            )
...
def run_probe(
    *,
    ...
    build_root = "/work/.tmp_fleetrmw_remote_liveliness_incompatible"
    install = f"{build_root}/install"
    network = f"fleetrmw-remote-liveliness-net-{os.getpid()}"
    ...
    return {
        ...
        "build_returncode": build.returncode,
        "remote_liveliness_incompatible_kind_event_production": ok,
        "remote_liveliness_incompatible_lease_event_production": ok,
        "remote_liveliness_incompatible_offered_direction": ok,
        "remote_liveliness_incompatible_requested_direction": ok,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=(
            "results_rmw_socket/"
            "docker_remote_liveliness_incompatible_event_probe_summary.json"
        ),
    )
