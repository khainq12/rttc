"""Build and run the FleetRMW remote type-hash-incompatible event probe in
separate Docker containers, over a real UDP wire with netem impairment.

Proves that remote type-incompatible-event detection now uses the RIHS
structural type hash (from get_type_hash_func) when both sides carry a
valid one, catching a same-declared-type-name-but-different-structure
mismatch that the prior plain type_name string comparison could not
detect -- while falling back to type_name equality when either side lacks
a valid hash, so this repository's own hand-built-type-support probes keep
working unmodified.
"""
...
SCHEMA_VERSION = "fleetrmw.docker_remote_type_hash_incompatible_event_probe.v1"
...
def observer_ok(probe: dict[str, Any]) -> bool:
    return (
        probe.get("status") == "ok"
        and probe.get("mismatch_ok") is True
        and probe.get("match_ok") is True
        and probe.get("fallback_ok") is True
    )
...
def run_case(
    *,
    ...
    observer = f"fleetrmw-remote-type-hash-observer-{os.getpid()}-{index}"
    advertiser = f"fleetrmw-remote-type-hash-advertiser-{os.getpid()}-{index}"
    binary = (
        f"{install}/rmw_fleetqox_cpp/lib/rmw_fleetqox_cpp/"
        "fleetrmw_remote_type_hash_incompatible_event_probe"
    )
    observer_command = (
        f"source /opt/ros/jazzy/setup.bash && source {install}/setup.bash && "
        "tc qdisc replace dev eth0 root netem delay 5ms 1ms && "
        f"FLEETQOX_RMW_BIND=0.0.0.0:48430 {binary} --mode observer --timeout-ms 9500"
    )
    start = run_command(
        [
            "docker",
    ...
    try:
        if ready:
            advertiser_command = (
                f"source /opt/ros/jazzy/setup.bash && source {install}/setup.bash && "
                "tc qdisc replace dev eth0 root netem delay 5ms 1ms && "
                "FLEETQOX_RMW_BIND=0.0.0.0:48431 "
                f"FLEETQOX_RMW_PEERS={observer}:48430 "
...
def run_probe(
    *,
    ...
    build_root = "/work/.tmp_fleetrmw_remote_type_hash_incompatible"
    install = f"{build_root}/install"
    network = f"fleetrmw-remote-type-hash-net-{os.getpid()}"
    ...
    return {
        ...
        "build_returncode": build.returncode,
        "remote_type_hash_mismatch_detection_claim": ok,
        "remote_type_hash_match_control_claim": ok,
        "remote_type_hash_no_hash_fallback_claim": ok,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=(
            "results_rmw_socket/"
            "docker_remote_type_hash_incompatible_event_probe_summary.json"
        ),
    )
