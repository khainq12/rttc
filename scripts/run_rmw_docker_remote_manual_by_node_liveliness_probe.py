"""Build and run the FleetRMW remote MANUAL_BY_NODE liveliness
assertion-sharing probe in separate Docker containers, over a real UDP wire
with netem impairment.

Proves that asserting one remote publisher renews every OTHER remote
MANUAL_BY_NODE publisher owned by the SAME remote node (grouped by
...
manual_by_node_liveliness_probe.cpp -- and that the shared lease still
expires normally once every publisher on that node goes idle.
"""
...
SCHEMA_VERSION = "fleetrmw.docker_remote_manual_by_node_liveliness_probe.v1"
...
def observer_ok(probe: dict[str, Any]) -> bool:
    return (
        ...
        and probe.get("remote_manual_by_node_assertion_sharing_claim") is True
        and probe.get("remote_manual_by_node_shared_lease_still_expires_claim") is True
    )
...
def run_case(
    *,
    ...
    observer = f"fleetrmw-remote-manual-by-node-observer-{os.getpid()}-{index}"
    advertiser = f"fleetrmw-remote-manual-by-node-advertiser-{os.getpid()}-{index}"
    binary = (
        f"{install}/rmw_fleetqox_cpp/lib/rmw_fleetqox_cpp/"
        "fleetrmw_remote_manual_by_node_liveliness_probe"
    )
    observer_command = (
        f"source /opt/ros/jazzy/setup.bash && source {install}/setup.bash && "
        "tc qdisc replace dev eth0 root netem delay 5ms 1ms && "
        f"FLEETQOX_RMW_BIND=0.0.0.0:48440 {binary} --mode observer --timeout-ms 9500"
    )
    start = run_command(
        [
            "docker",
    ...
    try:
        if ready:
            advertiser_command = (
                f"source /opt/ros/jazzy/setup.bash && source {install}/setup.bash && "
                "tc qdisc replace dev eth0 root netem delay 5ms 1ms && "
                "FLEETQOX_RMW_BIND=0.0.0.0:48441 "
                f"FLEETQOX_RMW_PEERS={observer}:48440 "
...
def run_probe(
    *,
    ...
    build_root = "/work/.tmp_fleetrmw_remote_manual_by_node"
    install = f"{build_root}/install"
    network = f"fleetrmw-remote-manual-by-node-net-{os.getpid()}"
    ...
    return {
        ...
        "build_returncode": build.returncode,
        "remote_manual_by_node_assertion_sharing_claim": ok,
        "remote_manual_by_node_shared_lease_still_expires_claim": ok,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=(
            "results_rmw_socket/"
            "docker_remote_manual_by_node_liveliness_probe_summary.json"
        ),
    )
