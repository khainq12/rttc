...
| Passing scoped claim boundaries | 608 / 654 |
...
The Docker-based integration probe suite under `scripts/run_rmw_docker_*.py`
(183 scripts, distinct from and not counted in the `tests/` unit/contract
suite above) passes 183/183 in a full, clean, sequential run of the entire
suite. Two defects are root-caused and fixed:
...
- an intermittent Nav2 SIGSEGV: a use-after-free where upstream `rcl`'s
  ...
  RMW's own node teardown, root-caused via a captured core dump and fixed
  by tracking live node pointers by identity rather than dereferencing a
  possibly-freed one;
...
The `router_multi_robot_qos_live_adaptive_matrix` check excludes
non-scheduler-decision rows and uses a tolerance with margin above the
measurement-noise ceiling (confirmed via a fifo-vs-fifo comparison, which
has no scheduler decision to regress, showing the same ~40ms swing).
...
in [Experimental Results](docs/EXPERIMENTAL_RESULTS_V1.md).
